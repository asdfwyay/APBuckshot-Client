...
func process_script(path: String, enable_hook_check := false, method_mask: Array[String] = []) -> String:
	...
	for method in moddable_methods:
		...
		var method_arg_string_names_only := get_function_arg_name_string(method.name, source_code, is_static)
...
static func get_function_arg_name_string(method_name: String, text: String, is_static: bool, offset := 0) -> String:
	var param_string := get_function_parameters(method_name, text, is_static, offset)
	var params := param_string.split(",")
	
	for i in params.size():
		params[i] = params[i].split(":")[0]

	var arg_string = ",".join(params)
